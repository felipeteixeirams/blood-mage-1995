...
- **Zustand (`src/store/gameStore.ts`)**: Armazena as estatísticas em tempo real do jogador (HP, Mana, Level, XP, Kills, Wave), inventário de itens de loot equipados, status de árvore de talentos de hemomancia, configurações do minimapa e volume de som.
