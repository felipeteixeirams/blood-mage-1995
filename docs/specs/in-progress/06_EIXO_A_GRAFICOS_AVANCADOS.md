...
| 2026-08-14 | Implementação (Frente 1): Adicionado `LightingPolish.ts` com Efeito de glow/bloom pontual para feitiços, itens raros, portais, monstros elites/bosses e eventos (morte, crítico, cura, level-up). Adicionado pulso vermelho para o cajado do Bloodmage via pointlight. Implementada transição de luz com ajuste de penumbra baseado na profundidade do andar (floorDepth) em `LightingSystem.ts`. | opencode (Felipe) |
| 2026-08-31 | Spec 6 (Eixos A, B, C): PostFX GPU (Shockwave radial displacement, pulso dinâmico de baixa vida, boss impact FX, CRT filter), Light2D (normal maps procedurais para paredes/baús/monstros, micro-flicker orgânico de tochas/braseiros) e Advanced Particles (gore temático por inimigo, rastro de dash espectral, faíscas de crítico). Testes 100% verdes. | Antigravity |
