  ...
  return (
    <div className="pointer-events-auto bg-[#0f0b09]/95 border-2 border-[#b8860b]/40 p-2 shadow-[4px_4px_12px_rgba(0,0,0,0.85)] backdrop-blur flex items-center gap-2.5 max-w-[280px] sm:max-w-xs">
      ...
      <div className="flex flex-col gap-1.5 flex-1 min-w-[150px]">
        ...
        {/* Active Status Effects & Buffs Row */}
        {(stats.statusConditions?.bleeding || stats.statusConditions?.poison || stats.statusConditions?.infection || isCriticalHp) && (
          <div className="flex flex-wrap gap-1 mt-1">
            {stats.statusConditions?.bleeding && (
              <span className="text-[7px] font-pixel px-1 py-0.5 bg-red-950/90 border border-red-700 text-red-300 rounded flex items-center gap-0.5 animate-pulse" title="Sangramento ativo: Dreno contínuo de HP">
                🩸 SANGRAMENTO
              </span>
            )}
            {stats.statusConditions?.poison && (
              <span className="text-[7px] font-pixel px-1 py-0.5 bg-emerald-950/90 border border-emerald-700 text-emerald-300 rounded flex items-center gap-0.5 animate-pulse" title="Envenenado: Redução de mobilidade">
                🍇 VENENO
              </span>
            )}
            {stats.statusConditions?.infection && (
              <span className="text-[7px] font-pixel px-1 py-0.5 bg-purple-950/90 border border-purple-700 text-purple-300 rounded flex items-center gap-0.5 animate-pulse" title="Infecção: Aumento de dano sofrido">
                🧪 INFECÇÃO
              </span>
            )}
            {isCriticalHp && (
              <span className="text-[7px] font-pixel px-1 py-0.5 bg-amber-950/90 border border-amber-500 text-amber-300 rounded flex items-center gap-0.5 animate-pulse" title="Fúria do Sangue: Dano amplificado em vida crítica!">
                ⚡ FÚRIA CRÍTICA
              </span>
            )}
          </div>
