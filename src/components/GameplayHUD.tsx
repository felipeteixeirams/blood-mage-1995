        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 pointer-events-auto">
          <div className="bg-[#0c0a09] border-4 border-double border-[#b8860b] shadow-[0_0_35px_rgba(0,0,0,0.95)] p-5 max-w-sm w-full relative flex flex-col gap-2.5 text-left">
            ...
            <div className="flex flex-col gap-1 py-1 border-b border-gray-900">
              <div className="flex justify-between items-center text-[9px] text-gray-300 uppercase">
                <span>VOLUME DA MÚSICA (BGM)</span>
                <span className="text-[#e8c76a] font-mono">{Math.round(settings.bgmVolume * 100)}%</span>
              </div>
              <input
                type="range"
                min="0"
                ...
                onChange={(e) => {
                  ...
                  updateSettings({ ...settings, bgmVolume: val });
                  soundEngine.setVolumes(settings.sfxVolume, val);
                }}
                className="w-full accent-[#b8860b] cursor-pointer h-1.5 bg-gray-800"
              />
            </div>
