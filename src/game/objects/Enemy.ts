...
export class Enemy extends Phaser.Physics.Arcade.Sprite {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number, monsterId: string) {
    ...
    this.setScale(monsterConfig.scale);
  ...
  public updateEnemy(
    ...
    if (this.config.gaitType === 'ethereal') {
      this.hoverOffsetY = Math.sin(time * 0.005) * 3;
      this.y += Math.sin(time * 0.008) * 0.2;
    }
    ...
    switch (this.aiState) {
      case 'idle':
        ...
        // Slowly look around
        this.facingAngle += Math.sin(time * 0.002) * 0.01;
        break;
      ...
      case 'patrol': {
        ...
        if (distP < 15) {
          ...
          const patrolSpeed = this.config.speed * 0.45;
      ...
      case 'investigating': {
        if (this.investigatePoint) {
          ...
          if (distInv > 20) {
            ...
            this.facingAngle = moveAngle;
            const invSpeed = this.config.speed * 0.65;
      ...
      case 'flee': {
        ...
        const fleeSpeed = this.config.speed * 1.2;
      ...
      case 'frenzy': {
        ...
        const currentSpeed = this.aiState === 'frenzy' ? this.config.speed * 1.35 : this.config.speed;
        ...
        switch (this.config.behavior) {
          ...
          case 'boss':
            if (distanceToPlayer > this.config.attackRange) {
              ...
              // Tactical backing away
              const backAngle = angleToPlayer + Math.PI;
              this.setVelocity(Math.cos(backAngle) * currentSpeed * 0.9, Math.sin(backAngle) * currentSpeed * 0.9);
            } else {
              // Tactical side strafe
              const strafeAngle = angleToPlayer + (Math.sin(time * 0.003) > 0 ? Math.PI / 2 : -Math.PI / 2);
              this.setVelocity(Math.cos(strafeAngle) * currentSpeed * 0.4, Math.sin(strafeAngle) * currentSpeed * 0.4);
            }
