...
import { GAME_ASSET_MANIFEST, queueAssetLoading, isAssetPhysicallyAvailable } from './assetManifest';
...
describe('assetManifest', () => {
  ...
  it('correctly identifies physically available assets on disk', () => {
    expect(isAssetPhysicallyAvailable('assets/sprites/player/bloodmage.png')).toBe(true);
    expect(isAssetPhysicallyAvailable('/public/assets/sprites/player/bloodmage.png')).toBe(true);
    expect(isAssetPhysicallyAvailable('assets/sprites/enemies/skeleton.png')).toBe(false);
    expect(isAssetPhysicallyAvailable('')).toBe(false);
  });

  it('filters out non-existent physical assets by default when loading GAME_ASSET_MANIFEST', () => {
    const mockLoader = {
      image: vi.fn(),
      spritesheet: vi.fn(),
      audio: vi.fn(),
  ...
  it('queues custom asset list directly and tracks metrics', () => {
