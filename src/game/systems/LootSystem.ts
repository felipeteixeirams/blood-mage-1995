...
export class LootSystem {
  ...
  public static generateLoot(floorDepth: number, isChest: boolean = false): LootItem {
    ...
    if (type === 'weapon') {
      ...
      stats.cooldownReductionBonus = parseFloat((0.03 * mult).toFixed(2));
      stats.maxHpBonus = Math.round(12 * mult * scaling);
      if (rarity === 'legendary' || rarity === 'epic') {
        stats.lifestealBonus = parseFloat((0.015 * mult).toFixed(3));
      }
      description = `Reduz Cooldowns em -${Math.round((stats.cooldownReductionBonus || 0) * 100)}%, concede +${stats.maxHpBonus} HP${stats.lifestealBonus ? ` e +${Math.round(stats.lifestealBonus * 100)}% Vampirismo` : ''}.`;
    }
