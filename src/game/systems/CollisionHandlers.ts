...
export class CollisionHandlers {
  ...
  public handleProjectileHitWall(projObj: any, wallObj: any) {
    ...
    // Wall blood splatter mark (persistent stain managed by BloodSplatterSystem)
    if (!proj.isEnemyProjectile) {
      if (scene.bloodSplatterSystem) {
        scene.bloodSplatterSystem.addWallSplatter(proj.x, proj.y, proj.rotation, false);
      } else {
  ...
  public handleTrapDamage(player: Player, damage: number) {
    ...
    // Player takes damage natively, triggerGameOver is not strictly needed here if takeDamage does it,
    // but typically player.takeDamage handles death.
  }

  public handleEnemyTrapDamage(enemy: Enemy, damage: number) {
    if (!enemy.active) return;
    // enemy.takeDamage(amount, sourceX, sourceY, isCrit, isExecution)
    enemy.takeDamage(damage, enemy.x, enemy.y, false, false);
  }
}
