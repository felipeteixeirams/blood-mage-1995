...
export class DismembermentSystem {
  ...
  public static executeDismemberment(
    ...
    if (result.type === 'total_destruction') {
      ...
      if (anyScene.bloodSplatterSystem) {
        anyScene.bloodSplatterSystem.addDeathBlood({
          x,
          y,
          monsterId: config.id,
          goreEffect: config.goreEffect,
          dismembermentType: 'total_destruction',
          killerSpellId,
          scaleMultiplier: isBoss ? 1.5 : 1.2,
        });
      } else {
      ...
      // 3. Medium floor blood pool & splatter (Frente 4)
      const anyScene = scene as any;
      if (anyScene.bloodSplatterSystem) {
        anyScene.bloodSplatterSystem.addDeathBlood({
          x,
        ...
        anyScene.bloodSplatterSystem.addCorpseDecal({
          x,
          y,
      ...
      // Standard small blood pool & intact corpse (Frente 4)
      const anyScene = scene as any;
      if (anyScene.bloodSplatterSystem) {
        anyScene.bloodSplatterSystem.addDeathBlood({
          x,
