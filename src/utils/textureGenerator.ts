...
export function generateGameTextures(scene: Phaser.Scene, options: TextureGenerationOptions = {}) {
  ...
  const addSpriteSheet = (key: string, canvas: HTMLCanvasElement, frameWidth: number, frameHeight: number) => {
    if (!options.force && scene.textures.exists(key)) {
      return;
    }
    if (scene.textures.exists(key)) {
    ...
    if (textureManager.addSpriteSheet) {
      textureManager.addSpriteSheet(key, canvas, { frameWidth, frameHeight });
    } else {
      scene.textures.addCanvas(key, canvas);
    }
  };
  ...
  const bloodmageCanvas = createPixelCanvas(544, 612, (ctx) => {
    ...
    const renderFrame = (originX: number, originY: number, dir: number, isWalk: boolean, step: number) => {
      const cx = originX + 34;
      const cy = originY + 34;
      ...
      ctx.beginPath();
      ctx.ellipse(cx, cy + 22, 13, 5, 0, 0, Math.PI * 2);
      ctx.fill();
      ...
      for (let y = robeTop; y <= robeBottom; y++) {
        ...
        const halfW = Math.round(5 + progress * 10);
      ...
      for (let y = torsoTop; y < beltY; y++) {
        ...
        for (let x = cx - halfW; x <= cx + halfW; x++) {
          ...
          else if (x === cx || x === cx - 1) col = (dir === 4) ? '#450a0a' : '#dc2626';
          ctx.fillStyle = col;
          ctx.fillRect(x + swayX, y, 1, 1);
        }
      }
      ...
      for (let y = headTop; y <= headBottom; y++) {
        ...
        for (let x = cx - halfW; x <= cx + halfW; x++) {
          ...
          else if (x === cx - halfW || x === cx + halfW) col = '#450a0a';
          ctx.fillStyle = col;
          ctx.fillRect(x + swayX, y, 1, 1);
        }
      }
      ...
      ctx.beginPath();
      ctx.arc(staffX + 1, staffY - 2, 4, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = '#ef4444';
      ctx.beginPath();
      ctx.arc(staffX + 1, staffY - 2, 2, 0, Math.PI * 2);
      ctx.fill();
    ...
    for (let dir = 0; dir < 8; dir++) {
      renderFrame(dir * FRAME_W, 0, dir, false, 0);
    }

    // Rows 1..8: Walk animations (8 frames per direction)
    for (let dir = 0; dir < 8; dir++) {
      const rowY = (1 + dir) * FRAME_H;
      for (let step = 0; step < 8; step++) {
        renderFrame(step * FRAME_W, rowY, dir, true, step);
      }
    }
  });
  ...
  // 15. Blood Pool Ground Stain (32x20) & Gore Decals (Frente 4)
  const bloodPoolCanvas = createPixelCanvas(32, 20, (ctx) => {
    // Outer organic puddle contour
    const grad = ctx.createRadialGradient(16, 10, 2, 16, 10, 14);
    grad.addColorStop(0, 'rgba(70, 5, 10, 0.92)'); // Coagulated dark core
    grad.addColorStop(0.65, 'rgba(150, 15, 25, 0.85)'); // Crimson arterial ring
    grad.addColorStop(1, 'rgba(110, 10, 18, 0.4)'); // Thinned edge

    ctx.fillStyle = grad;
    ctx.beginPath();
    ctx.ellipse(16, 10, 13, 7.5, -0.1, 0, Math.PI * 2);
    ctx.fill();

    // Secondary overlapping lobe for irregular puddle shape
    ctx.beginPath();
    ctx.ellipse(19, 12, 8, 5, 0.3, 0, Math.PI * 2);
    ctx.fill();

    ctx.beginPath();
    ctx.ellipse(12, 8, 7, 4.5, -0.25, 0, Math.PI * 2);
    ctx.fill();

    // Satellite blood droplets
    ctx.fillStyle = 'rgba(130, 12, 20, 0.8)';
    ctx.fillRect(4, 14, 2, 2);
    ctx.fillRect(27, 6, 2, 2);
    ctx.fillRect(8, 4, 1.5, 1.5);
    ctx.fillRect(24, 15, 1.5, 1.5);
  });
  addTexture('blood_pool_stain', bloodPoolCanvas);

  // 15b. Massive Blood Pool Decal (48x32) — Total destruction & Boss kills
  const bloodPoolLargeCanvas = createPixelCanvas(48, 32, (ctx) => {
    const grad = ctx.createRadialGradient(24, 16, 3, 24, 16, 22);
    grad.addColorStop(0, 'rgba(50, 3, 6, 0.95)'); // Very dark thick center
    grad.addColorStop(0.55, 'rgba(135, 10, 20, 0.88)');
    grad.addColorStop(0.85, 'rgba(180, 20, 30, 0.75)');
    grad.addColorStop(1, 'rgba(100, 8, 14, 0.3)');

    ctx.fillStyle = grad;
    // Central mass
    ctx.beginPath();
    ctx.ellipse(24, 16, 20, 12, 0.1, 0, Math.PI * 2);
    ctx.fill();

    // Multi-lobe splatter protrusions
    ctx.beginPath();
    ctx.ellipse(15, 12, 11, 7, -0.35, 0, Math.PI * 2);
    ctx.fill();

    ctx.beginPath();
    ctx.ellipse(33, 19, 12, 8, 0.25, 0, Math.PI * 2);
    ctx.fill();

    ctx.beginPath();
    ctx.ellipse(26, 23, 10, 6, -0.15, 0, Math.PI * 2);
    ctx.fill();

    // Satellite droplets & spray trails
    ctx.fillStyle = 'rgba(140, 12, 22, 0.85)';
    ctx.fillRect(6, 8, 3, 3);
    ctx.fillRect(3, 21, 2, 2);
    ctx.fillRect(41, 10, 2.5, 2.5);
    ctx.fillRect(39, 25, 3, 2);
    ctx.fillRect(19, 4, 2, 2);
    ctx.fillRect(31, 5, 2, 2);
  });
  addTexture('blood_pool_large', bloodPoolLargeCanvas);

  // 15c. Small Blood Splatter (16x16) — Projectile hits, wall marks, light cuts
  const bloodSplatterSmallCanvas = createPixelCanvas(16, 16, (ctx) => {
    ctx.fillStyle = 'rgba(145, 15, 25, 0.85)';
    // Central impact dot
    ctx.beginPath();
    ctx.arc(8, 8, 3.5, 0, Math.PI * 2);
    ctx.fill();

    // Surrounding scatter dots
    ctx.fillStyle = 'rgba(165, 20, 30, 0.8)';
    ctx.fillRect(3, 4, 2, 2);
    ctx.fillRect(12, 5, 1.5, 1.5);
    ctx.fillRect(11, 11, 2, 2);
    ctx.fillRect(4, 11, 1.5, 1.5);
    ctx.fillRect(8, 2, 1, 2);
    ctx.fillRect(14, 9, 1, 1);
  });
  addTexture('blood_splatter_small', bloodSplatterSmallCanvas);

  // 15d. Directional Arterial Spray / Slash Splatter (32x24)
  const bloodSplatterDirCanvas = createPixelCanvas(32, 24, (ctx) => {
    // Fan-shaped arterial spray expanding from left (origin) to right (spread)
    const grad = ctx.createLinearGradient(4, 12, 30, 12);
    grad.addColorStop(0, 'rgba(80, 5, 10, 0.95)');
    grad.addColorStop(0.4, 'rgba(160, 18, 28, 0.85)');
    grad.addColorStop(1, 'rgba(140, 12, 20, 0.5)');
    ...
    // Secondary fine spray droplets
    ctx.fillStyle = 'rgba(170, 20, 32, 0.85)';
    ctx.fillRect(29, 6, 2, 2);
    ctx.fillRect(30, 14, 1.5, 1.5);
    ctx.fillRect(27, 21, 2, 2);
    ctx.fillRect(18, 2, 1.5, 1.5);
    ctx.fillRect(20, 21, 1.5, 1.5);
    ctx.fillRect(12, 17, 2, 2);
  });
  addTexture('blood_splatter_directional', bloodSplatterDirCanvas);

  // 15e. Gore Chunk Decal (16x16) — Severed flesh & bone fragment
  const goreChunkCanvas = createPixelCanvas(16, 16, (ctx) => {
    // Flesh chunk
    ctx.fillStyle = '#450a0a'; // Dark coagulated base
    ctx.beginPath();
    ctx.ellipse(8, 9, 5, 3.5, 0.2, 0, Math.PI * 2);
    ctx.fill();

    ctx.fillStyle = '#7f1d1d'; // Crimson meat
    ctx.beginPath();
    ctx.ellipse(7, 8, 3.5, 2.5, -0.1, 0, Math.PI * 2);
    ctx.fill();

    // Bone splinter sticking out
    ctx.fillStyle = '#e2e8f0'; // Bone ivory
    ctx.fillRect(9, 6, 4, 2);
    ctx.fillStyle = '#94a3b8'; // Bone shadow
    ctx.fillRect(12, 6, 1, 2);
  });
  addTexture('gore_chunk_decal', goreChunkCanvas);

  // 15f. Bone Dust & Bone Fragments Decal (16x16) — Skeletons & bone hits
  const boneDustCanvas = createPixelCanvas(16, 16, (ctx) => {
    ctx.fillStyle = 'rgba(226, 232, 240, 0.75)'; // Ivory bone dust
    ctx.beginPath();
    ctx.ellipse(8, 8, 5, 3.5, 0.1, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.fillRect(6, 10, 2, 1.5);

    // Ash / dust scatter
    ctx.fillStyle = 'rgba(148, 163, 184, 0.6)';
    ctx.fillRect(3, 5, 1.5, 1.5);
    ctx.fillRect(12, 9, 1.5, 1.5);
  ...
  // 19d. Item Drops (Weapon, Armor, Relic)
  const createItemIcon = (type: 'weapon' | 'armor' | 'relic') => {
  ...
  addTexture('spr_item_weapon', createItemIcon('weapon'));
  addTexture('spr_item_armor', createItemIcon('armor'));
  addTexture('spr_item_relic', createItemIcon('relic'));

  
  // 19e. Traps and Barrels
  addTexture('spr_barrel', createPixelCanvas(32, 32, (ctx) => {
    // Wooden base
  ...
  const drawSpikeBase = (ctx: CanvasRenderingContext2D) => {
    ctx.fillStyle = '#1e293b'; // slate dark
    ctx.fillRect(2, 2, 28, 28);
    ctx.fillStyle = '#0f172a'; // holes
    for(let i=0; i<3; i++) {
      for(let j=0; j<3; j++) {
        ctx.fillRect(6 + i*8, 6 + j*8, 4, 4);
      }
    }
  };

  addTexture('spr_spike_hidden', createPixelCanvas(32, 32, (ctx) => {
    drawSpikeBase(ctx);
  }));

  addTexture('spr_spike_warn', createPixelCanvas(32, 32, (ctx) => {
    drawSpikeBase(ctx);
    ctx.fillStyle = '#94a3b8'; // tip of spike
    for(let i=0; i<3; i++) {
      for(let j=0; j<3; j++) {
        ctx.fillRect(7 + i*8, 7 + j*8, 2, 2);
      }
    }
  }));

  addTexture('spr_spike_active', createPixelCanvas(32, 32, (ctx) => {
    drawSpikeBase(ctx);
    ctx.fillStyle = '#cbd5e1'; // metallic spike
    for(let i=0; i<3; i++) {
      for(let j=0; j<3; j++) {
        ctx.fillRect(6 + i*8, 6 + j*8, 4, 4);
        ctx.fillStyle = '#f87171'; // bloody tip
        ctx.fillRect(7 + i*8, 7 + j*8, 2, 2);
        ctx.fillStyle = '#cbd5e1'; 
      }
    }
  ...
  const hazeCanvas = createPixelCanvas(128, 128, (ctx) => {
    for (let i = 0; i < 140; i++) {
      const fx = Math.random() * 128;
      const fy = Math.random() * 128;
      const fr = 14 + Math.random() * 26;
      const alpha = 0.008 + Math.random() * 0.018;

      const grad = ctx.createRadialGradient(fx, fy, 0, fx, fy, fr);
      grad.addColorStop(0, `rgba(240, 245, 255, ${alpha})`);
      grad.addColorStop(0.7, `rgba(240, 245, 255, ${alpha * 0.3})`);
